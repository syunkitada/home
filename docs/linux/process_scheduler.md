...
- スレッドの状態
  ...
  - S: 割り込み可能なイベント待ち
    ...
    - kill などでシグナル送れる
  - D: 割り込み不可なイベント待ち
    - ユーザーモードではなく、カーネルモードで実行されているときに見ることができるステータスです
      - disk io など
    - この状態にいるタスクに例えば kill コマンドでシグナルを送っても、割不可なのでシグナルの受信は今実行中の処理が終わって D 状態を抜けるまで待たされる
    - D 状態が一定以上長く滞留しているとハングしているとみなされる
    - D 状態というのは、カーネル内部で必ず何らかの排他制御が関わっています
    - D 状態に割り込みができない理由は、取得したロックを正しく開放するためです
      - ロックを取ったあとにシグナルを受け取ってタスクの他の部分の処理へ飛べてしまうと、ロックを正しく解放できない可能性が出てきます
    - 参考: [Linux でのハングタスクについて](https://rheb.hatenablog.com/entry/2023/03/13/135419)
