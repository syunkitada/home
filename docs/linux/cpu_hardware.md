  ...
  - 特殊レジスタ
    ...
    - 代表的な特殊レジスタ
      - プログラムカウンタ
        ...
        - しかし、命令サイクルを一つずつ実行する必要はなく、ある命令を実行している間に次の命令の fetch を行うというように効率よく進めることも可能
...
- パイプライン処理
  - 命令サイクルを一つずつ実行する必要はなく、ある命令を実行している間に次の命令の fetch を行うというように、各工程を流れ作業のように処理すること
- スーパースカラ
...
### 対策コードを無効にする（非推奨・検証限定）

以下はSpectre/Meltdown系のCPU脆弱性対策を無効化する例です。保護が低下するため、本番環境やインターネットに接続された環境では実行しないでください。性能比較などの隔離された検証環境で、影響と再有効化方法を確認したうえで使用します。現在のカーネルでは設定方法や項目が異なる場合があります。
